...
describe('Exchange', () => {
  let deployer, feeAccount, user1, user2, exchange, token1, token2, transaction;
  ...
  beforeEach(async () => {
    ...
    token1 = await Token.deploy('CoboltBlu', "BLU", '1000000');
    token2 = await Token.deploy('Cobolt USD', "CUSD", '1000000');
  ...
  describe('Withdrawing Tokens', () => {
    ...
    describe('Failure', () => {
      it('fails for insufficient balances', async () => {
        //Attempt to withdraw tokens withount depositing
        await expect(exchange.connect(user1).depositToken(token1.address, amount)).to.be.reverted;
      })
    })  
  })
